"""check_structure — verify finance-agent blueprint files are present."""
...
def run(target: Path) -> list[str]:
    ...
    if cfg.exists():
        try:
            ...
            if data.get("blueprint") != "finance-agent":
                failures.append(
                    f"harness.config.json blueprint is {data.get('blueprint')!r}, expected 'finance-agent'"
                )
