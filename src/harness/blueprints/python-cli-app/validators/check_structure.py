"""check_structure — verify python-cli-app blueprint files are present."""
...
EXPECTED_SKILLS = ["add-cli-command", "add-unit-test", "manage-dependency", "check-style"]
...
def run(target: Path) -> list[str]:
    ...
    if cfg.exists():
        try:
            ...
            if data.get("blueprint") != "python-cli-app":
                failures.append(
                    f"harness.config.json blueprint is {data.get('blueprint')!r}, expected 'python-cli-app'"
                )
